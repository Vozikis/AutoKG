...
CHECKPOINT_PATH  = "wd-movies.pt"

dataset_name = "wd-movies"               
...
for epoch in range(start_epoch, total_epochs):
    ...
    if (epoch + 1) % eval_every == 0 or (epoch + 1) == total_epochs:
        ...
        with torch.no_grad():
            for triples, _ in val_loader:
                preds += generate(model, triples.to(device), beam_width)
                for t in triples:
                    gts.append([tuple(edge.tolist()) for edge in t if edge[1] != PAD_RID])
        strict = strict_graph_accuracy(preds, gts); strict_acc.append(strict)
        print(f"          strict graph acc. (val): {strict:5.2f}%\n")
    else:
        strict_acc.append(strict_acc[-1] if strict_acc else 0.)
...
#     generated_graphs,
#     [list(map(tuple, t)) for t in next(iter(test_loader))[0].numpy()]
# )
...
def count_unique_graphs(model, num_samples=1000, beam=1):
    model.eval()
    latent_dim = model.enc.mu.out_features  # should be 128
...
_ = count_unique_graphs(model, num_samples=1000, beam=1)
